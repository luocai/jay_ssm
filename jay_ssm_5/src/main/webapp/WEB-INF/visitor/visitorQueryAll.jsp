				    	...
				    	<div class = "col-md-6 col-md-offset-6">
					    	<ul class = "pagination">
						    	...
						    	<c:if test="${page.currentPage == 1 }">
						    		<li class="disabled"><a href = "visitorQueryAll?currentPage=${page.currentPage-1 }">&laquo;</a> </li>
						    		<c:forEach begin = "${page.start }" end = "${page.end }" step = "1" var = "i">
						    			<c:if test="${i == page.currentPage }">
						    	       <li class="active"><a href = "visitorQueryAll?currentPage=${i }">${i }</a> </li>
						    			</c:if>
						    			<c:if test="${i != page.currentPage }">
						    		   <li>	<a href = "visitorQueryAll?currentPage=${i }">${i }</a></li>
						    			</c:if>
						    		</c:forEach>
						    		<li><a href = "visitorQueryAll?currentPage=${page.currentPage+1 }">&raquo;</a></li>
						    	</c:if>
						    	<!-- 当前页是中间页 -->
						    	<c:if test="${page.currentPage > 1 && page.currentPage < page.totalPage }">
						    		<li><a href = "visitorQueryAll?currentPage=${page.currentPage-1 }">&laquo;</a> </li>
						    		<c:forEach begin = "${page.start }" end = "${page.end }" step = "1" var = "i">
						    	...
						    	<c:if test="${page.currentPage == page.totalPage }">
						    		...
						    		<c:forEach begin = "${page.start }" end = "${page.end }" step = "1" var = "i">
						    			<c:if test="${page.currentPage == i }">
						    		<li class="active"><a href = "qvisitorQueryAll?currentPage=${i }">${i }</a> </li>
						    			</c:if>
						    			<c:if test="${i != page.currentPage }">
						    		<li><a href = "visitorQueryAll?currentPage=${i }">${i }</a></li>
						    			</c:if>
						    		</c:forEach>
						    		<li class="disabled"><a href = "visitorQueryAll?currentPage=${page.currentPage+1 }">&raquo;</a></li>
						    	</c:if>
						    <li> <a href = "visitorQueryAll?currentPage=${page.totalPage }">尾页</a> </li>
					    	</ul>
					    </div>
			    	</div>
		    	</div>
